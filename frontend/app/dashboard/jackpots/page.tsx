  ...
  return (
    <div className="bg-white shadow rounded-lg">
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
          ...
          {loading && <div className="text-lg text-gray-600 animate-pulse">Loading jackpots...</div>}
                  ...
                  {jackpots.map((jackpot, idx) => (
                    ...
                    >
                      <td className="px-4 py-3 text-blue-600 font-semibold">
                        <Link href={`/dashboard/jackpots/${jackpot.id}`} className="hover:underline">
                          {jackpot.name}
                        </Link>
                      </td>
                      <td className="px-4 py-3 text-gray-700">{jackpot.total_matches}</td>
                      <td className="px-4 py-3 text-green-600 font-semibold">Ksh {jackpot.current_amount.toLocaleString()}</td>
                      <td className="px-4 py-3 text-gray-600">{new Date(jackpot.scraped_at).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
